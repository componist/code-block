...
<div class="py-6">
    <div
        id="code-block-builder"
        x-data="templateBuilder()"
        x-init="init()"
        class="flex min-h-[400px] gap-4 p-4"
    >
        {{-- Code-Blöcke (links) --}}
        <div class="w-72 shrink-0 max-h-[calc(100vh-8rem)] overflow-y-auto rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <h3 class="mb-4 text-lg font-semibold text-gray-900">
                Code-Blöcke
                <span class="text-sm font-normal text-gray-500" x-show="blocks.length" x-text="'(' + blocks.length + ')'"></span>
            </h3>
            ...
            <div class="space-y-3" x-show="!blocksLoading && selectedCategoryId">
                <template x-for="block in blocks" :key="block.id">
                    <div
                        ...
                        @dragstart="dragStartBlock($event, block)"
        ...
        <div class="min-w-0 flex-1 overflow-y-auto rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <div class="mb-4 flex items-center justify-between">
                ...
                <button
                    x-show="templateBlocks.length > 0"
                    @click="openSaveModal()"
                    type="button"
            ...
            <div
                class="min-h-[320px] rounded-lg border-2 border-dashed border-gray-300 p-4 transition-colors"
                :class="{ 'flex items-center justify-center': templateBlocks.length === 0, 'border-indigo-400 bg-indigo-50/50': dragOverTemplate }"
            ...
            >
                <template x-if="templateBlocks.length === 0">
                    <p class="text-center text-gray-500">Blöcke per <strong>Klick</strong> oder <strong>Drag &amp; Drop</strong> hierher ziehen.</p>
                </template>
                <div class="space-y-4" x-show="templateBlocks.length > 0">
                    <template x-for="(block, index) in templateBlocks" :key="index">
                        <div
                            class="cursor-grab active:cursor-grabbing overflow-hidden rounded-lg border border-gray-200 bg-gray-50 transition-shadow hover:shadow-sm"
                            :class="{ 'ring-2 ring-indigo-400': dragOverIndex === index }"
                            draggable="true"
                            @dragstart="dragStartReorder($event, index)"
                        ...
                        >
                            ...
                            <div class="flex items-center justify-between gap-2 p-3">
                                <span class="min-w-0 flex-1 truncate text-sm font-medium text-gray-900" x-text="block.title || 'Block'"></span>
                                <div class="flex shrink-0 gap-1">
                                    <button type="button" x-show="index > 0" @click.stop="moveUp(index)" class="rounded p-1 text-gray-500 hover:bg-gray-200" title="Nach oben">↑</button>
                                    <button type="button" x-show="index < templateBlocks.length - 1" @click.stop="moveDown(index)" class="rounded p-1 text-gray-500 hover:bg-gray-200" title="Nach unten">↓</button>
                                    <button type="button" @click.stop="removeBlock(index)" class="rounded p-1 text-red-600 hover:bg-red-50" title="Entfernen">✕</button>
                                </div>
                            </div>
                        </div>
                    </template>
            ...
            <div class="space-y-2" x-show="!loading">
                <template x-for="cat in categories" :key="cat.id">
                    <button
                        ...
                        :class="selectedCategoryId === cat.id ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'"
                        x-text="cat.name"
                    ></button>
                </template>
            </div>
        ...
        >
            ...
            >
                ...
                <p class="mb-2 text-sm text-gray-600">Dateiname (ohne .blade.php): <code class="rounded bg-gray-100 px-1" x-text="(templateName || '').trim() || '…'"></code></p>
                <input
                    type="text"
                    x-model="templateName"
                    placeholder="z. B. hero-page"
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" @click="closeSaveModal()" class="rounded-lg bg-gray-200 px-4 py-2 text-gray-800 hover:bg-gray-300">Abbrechen</button>
                    <button type="button" @click="save()" :disabled="saving" class="rounded-lg bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-700 disabled:opacity-50">
